...
#include "apps/voip/VoIPSenderUe.h"

#define round(x) floor((x) + 0.5)

Define_Module(VoIPSenderUe);
using namespace inet;



VoIPSenderUe::VoIPSenderUe()
{
    selfSource_ = nullptr;
    selfSender_ = nullptr;
}
...
void VoIPSenderUe::initialize(int stage)
{
    ...
    batteryVector.setName("BatteryLevel");
    batteryTime_ = 172800;
    battery_ = 240.0;
    endTime_ = 0;
    isNewPkt_ = true;
...
void VoIPSenderUe::handleMessage(cMessage *msg)
{
...
void VoIPSenderUe::talkspurt(simtime_t dur)
{
    iDtalk_++;
...
void VoIPSenderUe::selectPeriodTime()
{
...
void VoIPSenderUe::sendVoIPPacket()
{
...
void VoIPSenderUe::batteryCalculator()
{
//    if(batteryTime_ <= 0){
//        getSimulation()->getActiveEnvir()->alert("Battery is equal/less than zero");
//        finish();
//    }

    if(isNewPkt_){

        simtime_t tempTime = durSil_;

        batteryTime_ = batteryTime_ - (simTime() - tempTime  - endTime_);
        EV << "Delta is: " <<simTime() - tempTime  - endTime_ << " Sec \n";
