...
#include "apps/IoT/IoTReceiver.h"

Define_Module(IoTReceiver);
...
void IoTReceiver::initialize(int stage)
{
    ...
    EV << "IoTReceiver::initialize - binding to port: local:" << port << endl;
    if (port != -1)
    {
...
void IoTReceiver::handleMessage(cMessage *msg)
{
    ...
    if (pPacket == 0)
    {
        throw cRuntimeError("IoTReceiver::handleMessage - FATAL! Error when casting to inet packet");
    }

    // read VoIP header
    auto voipHeader = pPacket->popAtFront<IoTPacket>();
    ...
    EV << "IoTReceiver::handleMessage - Packet received: TALK[" << voipHeader->getIDtalk() << "] - FRAME[" << voipHeader->getIDframe() << " size: " << voipHeader->getChunkLength() << " bytes]\n";
...
void IoTReceiver::playout(bool finish)
{
    ...
    IoTPacket* pPacket = mPacketsList_.front();
    ...
    while (!mPacketsList_.empty())
    {
        ...
        if(!finish)
            EV << "IoTReceiver::playout - Jitter measured: " << last_jitter << " TALK[" << pPacket->getIDtalk() << "] - FRAME[" << IDframe << "]\n";
        ...
        if (IDframe < n_frames)
        {
            ...
            if (isArrived[IDframe])
            {
                // avoid printing during finish (as it will print to the standard output)
                if(!finish)
                    EV << "IoTReceiver::playout - Duplicated Packet: TALK[" << pPacket->getIDtalk() << "] - FRAME[" << IDframe << "]\n";
                delete pPacket;
            }
            else if( last_jitter > 0.0 )
            {
                ...
                // avoid printing during finish (as it will print to the standard output)
                if(!finish)
                    EV << "IoTReceiver::playout - out of time packet deleted: TALK[" << pPacket->getIDtalk() << "] - FRAME[" << IDframe << "]\n";
            ...
            else
            {
                ...
                if(mBufferSpace_ > 0)
                {
                    ...
                    if(!finish)
                    {
                        EV << "IoTReceiver::playout - Sampleable packet inserted into buffer: TALK["<< pPacket->getIDtalk() << "] - FRAME[" << IDframe
                ...
                else
                {
                    ...
                    if(!finish)
                    {
                        EV << "IoTReceiver::playout - Buffer is full, discarding packet: TALK[" << pPacket->getIDtalk() << "] - FRAME["
    ...
    if(!finish)
    {
        EV << "IoTReceiver::playout - proportionalLoss " << proportionalLoss << "(tailDropLoss=" << tailDropLoss << " - playoutLoss="
    ...
    if(!finish)
    {
        EV << "IoTReceiver::playout - Computed MOS: eModel( " << mPlayoutDelay_ << " , " << tailDropLoss << "+" << playoutLoss << "+"
           << channelLoss << " ) = " << mos << "\n";

        EV << "IoTReceiver::playout - Playout Delay Adaptation \n" << "\t Old Playout Delay: " << mPlayoutDelay_ << "\n\t Max Jitter Measured: "
           << max_jitter << "\n\n";
    }
...
double IoTReceiver::eModel(simtime_t delay, double loss)
{
